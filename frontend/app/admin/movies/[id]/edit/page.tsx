...
import { adminAPI, movieAPI } from '@/lib/api';
...
export default function EditMovie() {
  ...
  useEffect(() => {
    ...
    const fetchMovie = async () => {
      try {
        setLoading(true);
        // Since we need to get by ID, we'll fetch all and find
        const res = await adminAPI.getMovies({ limit: 1000 });
        const movie = res.data.movies.find((m: Movie) => m._id === id);
        if (movie) {
          setFormData(movie);
        }
      } catch (err) {
  ...
  if (!formData) {
    return <div className="text-center py-12 text-red-500">{error || 'Movie not found'}</div>;
  }
  ...
  return (
    <div className="min-h-screen bg-black p-8">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-4xl font-bold text-white mb-8">Edit Movie</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="bg-gray-900 p-6 rounded-lg">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-gray-300 mb-2">Clean Title</label>
                <input
                  type="text"
                  name="cleanTitle"
              ...
              <div>
                <label className="block text-gray-300 mb-2">Full Title</label>
                <textarea
                  name="title"
